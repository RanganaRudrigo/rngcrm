...
class Base_Model extends CI_Model
{
    ...
    public function relate($row)
    {
        ...
        foreach ($this->belongs_with as $key => $value)
        {
            if (is_string($value))
            {
                $relationship = $value;
            ...
            if (in_array($relationship, $this->_with))
            {
                ...
                if (is_object($row))
                {
                    $row->{$relationship} = $this->{$relationship . '_model'}->get_by($options['key'],$row->{$options['key']});
                }
                else
                {
                    $row[$relationship] = $this->{$relationship . '_model'}->get_by($options['key'],$row->{$options['key']});
    ...
    protected function _set_where($params)
    {
        ...
        else if(count($params) == 2)
        {
            ...
            {
                $this->_database->where_in($this->_table.".".$params[0], $params[1]);
            }
            else
            {
                $this->_database->where($this->_table.".".$params[0], $params[1]);
            }
        }
