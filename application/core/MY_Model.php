...
class MY_Model extends Base_Model
{
    ...
    function join($table){
        if(!empty($this->has_one[$table])) {
            $this->_database->join($table, "{$this->_table}.{$this->has_one[$table][0]} = $table.{$this->has_one[$table][1]} " );
        }
        return $this;
    }
    ...
    function prop_data_before_create($data){
        $data['CreatedBy'] = $this->session->userdata("user")->user_id ;
        $data['CreatedDate'] = date("Y-m-d H:i:s");
        $data['ModifiedBy'] = $this->session->userdata("user")->user_id ;
